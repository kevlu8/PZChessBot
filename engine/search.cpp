...
Move next_move(pzstd::vector<std::pair<Move, Value>> &moves, int &end_idx) {
	if (end_idx == 0) return NullMove;
	int idx = 0;
	Move best = NullMove;
...
Value quiesce(Board &board, int side, Value alpha, Value beta, int ply = 0) {
	...
	Move m = NullMove;
	int end_idx = scores.size();
	while ((m = next_move(scores, end_idx)) != NullMove) {
		board.make_move(m);
...
Value negamax(Board &board, int depth, int side, bool pv_node, int ply = 0, Value alpha = -VALUE_INFINITE, Value beta = VALUE_INFINITE) {
	...
	Move m = NullMove;
	int m_idx = 0, end_idx = scores.size();
	while ((m = next_move(scores, end_idx)) != NullMove) {
		line[ply+1].move = m;
		board.make_move(m);
		Value score = 0;
