...
pzstd::vector<bool> thread_begin;
...
void thread_loop(ThreadInfo &ti) {
	while (!quit) {
		...
		search_cv.wait(lk, [ti] { return (thread_begin.size() && thread_begin[ti.id]) || quit; });
		if (quit) {
			lk.unlock();
			break;
		}

		thread_begin[ti.id] = false;
		lk.unlock();
		iterativedeepening(ti, ti.depth);

		if (running.fetch_sub(1, std::memory_order_relaxed) == 1) {
...
std::pair<Move, Value> search(Board &board, ThreadInfo *threads, int64_t time, int depth, int64_t maxnodes, int quiet) {
	...
	for (int t = 0; t < num_threads; t++) {
		...
		ti.depth = depth;
		nodes[t] = 0;
		if (thread_begin.size() < num_threads)
			thread_begin.push_back(true);
		else
			thread_begin[t] = true;
	}
	running = num_threads;
	...
	{
		std::unique_lock<std::mutex> lk(search_mtx);
		search_cv.wait(lk, [] { return quit || running == 0; });
	}
