...
Value __recurse(Board &board, int depth, Value alpha = -VALUE_INFINITE, Value beta = VALUE_INFINITE, int side = 1, bool pv = false, int ply = 1, bool cutnode = false) {
	pvlen[ply] = 0;
	...
	if (!in_check && npieces != npawns_and_kings && cur_eval >= beta) { // Avoid NMP in pawn endgames
		...
		Value null_score = -__recurse(board, depth - NMP_R_VALUE, -beta, -beta + 1, -side, pv, ply+1, true);
		board.unmake_move();
		if (null_score >= beta)
			return null_score;
	}
	...
	while ((move = next_move(scores, end)) != NullMove) {
		...
		if (line[ply].excl == NullMove && depth >= 8 && tentry && move == tentry->best_move && tentry->depth >= depth - 2 && tentry->flags != UPPER_BOUND) {
			...
			Value singular_score = __recurse(board, (depth-1) / 2, singular_beta - 1, singular_beta, side, 0, ply, false);
			...
			if (singular_score < singular_beta) {
				...
				// expected cutnode but tt move is not expected to fail high
				extension = -2;
			}
		}
		...
		if (i > 0) {
			...
			score = -__recurse(board, depth - r / 1024, -alpha - 1, -alpha, -side, 0, ply+1, !cutnode);
			if (score > alpha) {
				score = -__recurse(board, depth - 1, -beta, -alpha, -side, pv, ply+1, !cutnode);
			}
		} else {
			score = -__recurse(board, depth - 1 + extension, -beta, -alpha, -side, pv, ply+1, !cutnode);
		}
...
std::pair<Move, Value> __search(Board &board, int depth, Value alpha = -VALUE_INFINITE, Value beta = VALUE_INFINITE, int side = 1) {
	...
	while ((move = next_move(scores, end)) != NullMove) {
		...
		Value score;
		if (i > 0) {
			score = -__recurse(board, depth - reduction[i][depth] / 1024, -alpha - 1, -alpha, -side, 0, 1, false);
			if (score > alpha) {
				score = -__recurse(board, depth - 1, -beta, -alpha, -side, 0, 1, false);
			}
		} else {
			score = -__recurse(board, depth - 1, -beta, -alpha, -side, 1, 1, false);
		}
