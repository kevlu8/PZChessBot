...
Value corrhist_pc[2][CORRHIST_SZ]; // [side][piece hash]
...
void update_corrhist(bool side, Board &board, Value diff, int depth) {
	const Value sdiff = diff * CORRHIST_GRAIN;
	const Value weight = std::min(depth*depth, 128);
	Value &pscorr = corrhist_ps[side][board.pawn_hash % CORRHIST_SZ];
	pscorr = std::clamp((pscorr * (CORRHIST_WEIGHT - weight) + sdiff * weight) / CORRHIST_WEIGHT, -MAX_HISTORY, (int)MAX_HISTORY);
	Value &matcorr = corrhist_mat[side][board.material_hash() % CORRHIST_SZ];
	matcorr = std::clamp((matcorr * (CORRHIST_WEIGHT - weight) + sdiff * weight) / CORRHIST_WEIGHT, -MAX_HISTORY, (int)MAX_HISTORY);
	Value &pccorr = corrhist_pc[side][board.piece_hash % CORRHIST_SZ];
	pccorr = std::clamp((pccorr * (CORRHIST_WEIGHT - weight) + sdiff * weight) / CORRHIST_WEIGHT, -MAX_HISTORY, (int)MAX_HISTORY);
}

void apply_correction(bool side, Board &board, Value &eval) {
	if (abs(eval) >= VALUE_MATE_MAX_PLY)
		return; // Don't apply correction if we are already at a mate score
	const Value pscorr = corrhist_ps[side][board.pawn_hash % CORRHIST_SZ];
	const Value matcorr = corrhist_mat[side][board.material_hash() % CORRHIST_SZ];
	const Value pccorr = corrhist_pc[side][board.piece_hash % CORRHIST_SZ];
	const Value corr = (pscorr + matcorr + pccorr) / 3;
...
Value quiesce(Board &board, Value alpha, Value beta, int side, int depth) {
	...
	apply_correction(board.side, board, stand_pat);
...
Value __recurse(Board &board, int depth, Value alpha = -VALUE_INFINITE, Value beta = VALUE_INFINITE, int side = 1, bool pv = false, int ply = 1) {
	...
	if (!in_check) {
		...
		raw_eval = cur_eval;
		apply_correction(board.side, board, cur_eval);
	}
	...
	for (int i = 0; i < moves.size(); i++) {
		...
		if (score >= beta) {
			...
			if (!capt) { // Not a capture
				...
				if (!in_check && !promo && best > raw_eval) update_corrhist(board.side, board, best - raw_eval, depth);
			} else { // Capture
	...
	if (!in_check && !best_iscapture && !best_ispromo && !(best < alpha && best >= raw_eval)) {
		...
		update_corrhist(board.side, board, best - raw_eval, depth);
	}
