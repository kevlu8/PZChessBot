...
void update_corrhist(SearchParams &params, bool side, uint64_t pshash, uint64_t mathash, uint64_t nphash, Move prev, Value diff, int depth) {
	...
	Value &movcorr = params.corrhist_prev[side][prev.src()][prev.dst()];
	movcorr = std::clamp((movcorr * (CORRHIST_WEIGHT - weight) + sdiff * weight) / CORRHIST_WEIGHT, -MAX_HISTORY, (int)MAX_HISTORY);
	Value &npcorr = params.corrhist_np[side][nphash % CORRHIST_SZ];
	npcorr = std::clamp((npcorr * (CORRHIST_WEIGHT - weight) + sdiff * weight) / CORRHIST_WEIGHT, -MAX_HISTORY, (int)MAX_HISTORY);
}

void apply_correction(SearchParams &params, bool side, uint64_t pshash, uint64_t mathash, uint64_t nphash, Move prev, Value &eval) {
	...
	const Value movcorr = params.corrhist_prev[side][prev.src()][prev.dst()];
	const Value npcorr = params.corrhist_np[side][nphash % CORRHIST_SZ];
 ...
 */
Value quiesce(Board &board, SearchParams &sp, BoardState &bs, Value alpha, Value beta, int side, int depth, bool pv=false) {
	sp.nodes++;
	...
	else stand_pat = eval(board, bs) * side;
	apply_correction(sp, board.side, board.pawn_struct_hash(), board.material_hash(), board.nonpawn_hash(), sp.line[depth-1].move, stand_pat);
	...
	for (int i = 0; i < scores.size(); i++) {
		...
		board.make_move(move);
		Value score = -quiesce(board, sp, bs, -beta, -alpha, -side, depth + 1, pv);
		board.unmake_move();
 ...
 */
pzstd::vector<std::pair<Move, Value>> assign_values(Board &board, SearchParams &sp, pzstd::vector<Move> &moves, int ply, TTable::TTEntry *tentry) {
	pzstd::vector<std::pair<Move, Value>> scores;
...
Value __recurse(Board &board, SearchParams &sp, BoardState &bs, int depth, Value alpha = -VALUE_INFINITE, Value beta = VALUE_INFINITE, int side = 1, bool pv = false, bool cutnode = false, int ply = 1) {
	sp.pvlen[ply] = 0;
	...
	if (depth <= 0) {
		...
		return quiesce(board, sp, bs, alpha, beta, side, ply, pv);
	}
	...
	if (!in_check) {
		...
		raw_eval = cur_eval;
		apply_correction(sp, board.side, pawn_hash, board.material_hash(), board.nonpawn_hash(), sp.line[ply-1].move, cur_eval);
	}
	...
	if (!in_check && npieces != npawns_and_kings && cur_eval >= beta && depth >= 2 && sp.line[ply].excl == NullMove) { // Avoid NMP in pawn endgames
		/**
		...
		Value null_score = -__recurse(board, sp, bs, depth - r, -beta, -beta + 1, -side, 0, !cutnode, ply+1);
		board.unmake_move();
		if (null_score >= beta)
			return null_score;
	}
	...
	pzstd::vector<std::pair<Move, Value>> scores = assign_values(board, sp, moves, ply, tentry);
	int end = scores.size();
	...
	while ((move = next_move(scores, end)) != NullMove) {
		...
		if (sp.line[ply].excl == NullMove && depth >= 8 && tentry && move == tentry->best_move && tentry->depth >= depth - 2 && tentry->flags != UPPER_BOUND) {
			...
			Value singular_score = __recurse(board, sp, bs, (depth-1) / 2, singular_beta - 1, singular_beta, side, 0, cutnode, ply);
		...
		if (depth >= 2 && i > 3) {
			...
			r -= 1024 * pv;
			r += 1024 * (!pv && cutnode);
			if (move == sp.killer[0][ply] || move == sp.killer[1][ply]) r -= 1024;

			Value searched_depth = depth - r / 1024;

			score = -__recurse(board, sp, bs, searched_depth, -alpha - 1, -alpha, -side, 0, true, ply+1);
			if (score > alpha && searched_depth < newdepth) {
				score = -__recurse(board, sp, bs, newdepth, -alpha - 1, -alpha, -side, 0, !cutnode, ply+1);
			}
		} else if (!pv || i > 0) {
			score = -__recurse(board, sp, bs, newdepth, -alpha - 1, -alpha, -side, 0, !cutnode, ply+1);
		}
		if (pv && (i == 0 || score > alpha)) {
			...
			score = -__recurse(board, sp, bs, newdepth, -beta, -alpha, -side, 1, false, ply+1);
		}
	...
	if (best == -VALUE_MATE) {
		...
		if (sp.line[ply].excl != NullMove) return alpha;
	...
	if (abs(best) < VALUE_MATE_MAX_PLY && !in_check && !best_iscapture && !best_ispromo && !(best < alpha && best >= raw_eval) && !(best >= beta && best <= raw_eval)) {
		...
		update_corrhist(sp, board.side, pawn_hash, board.material_hash(), board.nonpawn_hash(), sp.line[ply-1].move, best - raw_eval, depth);
	}
...
std::pair<Move, Value> __search(Board &board, SearchParams &sp, BoardState &bs, int depth, Value alpha = -VALUE_INFINITE, Value beta = VALUE_INFINITE, int side = 1) {
	...
	TTable::TTEntry *tentry = board.ttable.probe(board.zobrist);
	pzstd::vector<std::pair<Move, Value>> scores = assign_values(board, sp, moves, 0, tentry);
	...
	while ((move = next_move(scores, end)) != NullMove) {
		...
		if (i > 3) {
			...
			score = -__recurse(board, sp, bs, searched_depth, -alpha - 1, -alpha, -side, 0, true);
			if (score > alpha && searched_depth < newdepth) {
				score = -__recurse(board, sp, bs, newdepth, -alpha - 1, -alpha, -side, 0, true);
			}
		} else if (i > 0) {
			score = -__recurse(board, sp, bs, newdepth, -alpha - 1, -alpha, -side, 0, true);
		}
		if (i == 0 || score > alpha) {
			score = -__recurse(board, sp, bs, newdepth, -beta, -alpha, -side, 1, false);
		}
...
void clear_search_vars(SearchParams &sp) {
	...
	for (int i = 0; i < MAX_PLY; i++) {
		sp.killer[0][i] = sp.killer[1][i] = NullMove;
		sp.pvlen[i] = 0;
		sp.line[i] = SSEntry();
	}
	for (int i = 0; i < 64; i++) {
		for (int j = 0; j < 64; j++) {
			sp.history[0][i][j] = sp.history[1][i][j] = 0;
			sp.corrhist_prev[0][i][j] = sp.corrhist_prev[1][i][j] = 0;
			sp.cmh[0][i][j] = sp.cmh[1][i][j] = NullMove;
		}
		for (int j = 0; j < 6; j++) {
			for (int k = 0; k < 6; k++) {
				sp.capthist[j][k][i] = 0;
	...
	for (int i = 0; i < CORRHIST_SZ; i++) {
		sp.corrhist_ps[0][i] = sp.corrhist_ps[1][i] = 0;
		sp.corrhist_mat[0][i] = sp.corrhist_mat[1][i] = 0;
		sp.corrhist_np[0][i] = sp.corrhist_np[1][i] = 0;
	}
}
