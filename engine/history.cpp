...
void History::update_corrhist(Board &board, int bonus) {
	...
	update_entry(corrhist_np[WHITE][board.side][board.nonpawn_hash(WHITE) % CORRHIST_SZ]);
	update_entry(corrhist_np[BLACK][board.side][board.nonpawn_hash(BLACK) % CORRHIST_SZ]);
}
...
void History::apply_correction(Board &board, Value &eval) {
	...
	corr += 128 * corrhist_np[WHITE][board.side][board.nonpawn_hash(WHITE) % CORRHIST_SZ];
	corr += 128 * corrhist_np[BLACK][board.side][board.nonpawn_hash(BLACK) % CORRHIST_SZ];
