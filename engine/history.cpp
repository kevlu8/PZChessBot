...
void History::update_history(Board &board, Move &move, int ply, SSEntry *line, Value bonus) {
	...
	update_conthist(board, move, ply, line, bonus);
}

void History::update_conthist(Board &board, Move &move, int ply, SSEntry *line, Value bonus) {
	int cbonus = std::clamp(bonus, (Value)(-MAX_HISTORY), MAX_HISTORY);
...
void Corrhist::update_corrhist(Board &board, SSEntry *line, int ply, int bonus) {
