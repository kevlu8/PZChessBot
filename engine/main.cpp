	...
	while (getline(std::cin, command)) {
		if (command == "uci") {
			...
			if (multipv != 1) {
				...
				if (inf)
					res = search(board, 1e9, MAX_PLY, 1e18, quiet, threads);
				else if (depth != -1)
					res = search(board, 1e9, depth, 1e18, quiet, threads);
				else if (nodes != -1)
					res = search(board, 1e9, MAX_PLY, nodes, quiet, threads);
				else if (movetime != -1)
					res = search(board, movetime, MAX_PLY, 1e18, quiet, threads);
				else
					res = search(board, timemgmt(timeleft, inc, online), MAX_PLY, 1e18, quiet, threads);
			}
