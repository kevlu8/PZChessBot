...
void TTable::store(uint64_t key, Value eval, Value s_eval, uint8_t depth, uint8_t bound, bool ttpv, Move best_move) {
	...
	uint16_t shortkey = key >> 48; // Use upper 16 bits for the key (since we already verified bottom n bits)
	
	TTEntry entry = bucket.entries[0];
	uint8_t idx = 0;

	if (entry.key != 0 && entry.key != shortkey) {
		for (uint8_t i = 1; i < 3; i++) {
			const TTEntry nentry = bucket.entries[i];
			if (nentry.key == shortkey) {
				entry = nentry;
				idx = i;
				break;
			}
	...
	if (best_move == NullMove && entry.key == shortkey)
	...
	if (entry.key != shortkey || (bound == EXACT && entry.bound() != EXACT) || new_prio * 3 >= existing_prio * 2) {
		entry.key = shortkey;
		entry.eval = eval;
		entry.s_eval = s_eval;
		entry.depth = depth;
