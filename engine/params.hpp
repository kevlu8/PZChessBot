...
TUNE(rfp_quad, 224, 1, 320, 16); // quantized to 32
TUNE(rfp_cutnode, 30, 5, 30, 5);
TUNE(rfp_corr, 112, 20, 250, 20);
TUNE(nmp_margin, 184, 50, 400, 20);
TUNE(nmp_depth, 8, 0, 25, 3);
TUNE(razor_margin, 231, 150, 400, 20);
TUNE(probcut_margin, 409, 200, 500, 30);
TUNE(se_base, 48, 1, 160, 16); // quantized to 32
TUNE(dext_base, 26, 10, 50, 5);
TUNE(dext_capt, 10, 0, 100, 5);
TUNE(dext_pv, 35, 0, 100, 5);
...
TUNE(hist_quad, 128, 0, 224, 16); // quantized to 32
TUNE(hist_lin, 139, 64, 256, 16);
TUNE(hist_const, 124, 0, 256, 16);
TUNE(postlmr_quad, 96, 0, 224, 16); // quantized to 32
TUNE(postlmr_lin, 126, 64, 256, 16);
...
TUNE(corr_threat, 90, 64, 256, 8);
TUNE(probcut_see, 62, 0, 200, 10);
