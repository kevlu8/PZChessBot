...
void Board::make_move(Move move) {
	...
	uint64_t old_piece_hash = piece_hash;
	recompute_hash();
	if (old_hash != zobrist || old_pawn_hash != pawn_hash || old_piece_hash != piece_hash) {
		...
		std::cerr << "Or, piece hash mismatch before move: expected " << piece_hash << " got " << old_piece_hash << '\n';
		abort();
	}
#endif
	...
	if (move.data != 0 && (piece_boards[OPPOCC(side)] & square_bits(move.dst()))) { // If opposite occupancy bit set on destination (capture)
		...
		} else piece_hash ^= zobrist_square[move.dst()][mailbox[move.dst()]];
	...
	if (move.data == 0) {
		...
		zobrist ^= zobrist_square[move.dst()][mailbox[move.dst()]];
		piece_hash ^= zobrist_square[move.dst()][mailbox[move.dst()]]; // can assume it is a piece because pawns cannot promote to pawns
		...
		if (move.data == 0b1100000100000110) {
			...
			piece_hash ^= zobrist_square[SQ_E1][Piece(WHITE_KING)] ^ zobrist_square[SQ_G1][Piece(WHITE_KING)];
			piece_hash ^= zobrist_square[SQ_H1][Piece(WHITE_ROOK)] ^ zobrist_square[SQ_F1][Piece(WHITE_ROOK)];
			...
			piece_hash ^= zobrist_square[SQ_E1][Piece(WHITE_KING)] ^ zobrist_square[SQ_C1][Piece(WHITE_KING)];
			piece_hash ^= zobrist_square[SQ_A1][Piece(WHITE_ROOK)] ^ zobrist_square[SQ_D1][Piece(WHITE_ROOK)];
			...
			piece_hash ^= zobrist_square[SQ_E8][Piece(BLACK_KING)] ^ zobrist_square[SQ_G8][Piece(BLACK_KING)];
			piece_hash ^= zobrist_square[SQ_H8][Piece(BLACK_ROOK)] ^ zobrist_square[SQ_F8][Piece(BLACK_ROOK)];
			...
			piece_hash ^= zobrist_square[SQ_E8][Piece(BLACK_KING)] ^ zobrist_square[SQ_C8][Piece(BLACK_KING)];
			piece_hash ^= zobrist_square[SQ_A8][Piece(BLACK_ROOK)] ^ zobrist_square[SQ_D8][Piece(BLACK_ROOK)];
		} else {
		...
		if (piece == PAWN) {
			...
			piece_hash ^= zobrist_square[move.src()][mailbox[move.src()]];
			piece_hash ^= zobrist_square[move.dst()][mailbox[move.src()]];
		}
		mailbox[move.dst()] = mailbox[move.src()];
		mailbox[move.src()] = NO_PIECE;
	...
	old_piece_hash = piece_hash;
	recompute_hash();
	if (zobrist != old_hash || pawn_hash != old_pawn_hash || piece_hash != old_piece_hash) {
		print_board();
		...
		std::cerr << "Or, piece hash mismatch after make: expected " << old_piece_hash << " got " << piece_hash << std::endl;
...
void Board::unmake_move() {
	...
	if (old_hash != zobrist || old_pawn_hash != pawn_hash || old_piece_hash != piece_hash) {
		...
		std::cerr << "Or, piece hash mismatch before unmake: expected " << piece_hash << " got " << old_piece_hash << '\n';
		abort();
	}
#endif
	...
	if (move.data == 0) {
		...
		piece_hash ^= zobrist_square[move.dst()][mailbox[move.dst()]] ^ zobrist_square[move.dst()][prev.prev_piece()];
		...
		if (piece == PAWN) {
			...
			piece_hash ^= zobrist_square[move.dst()][mailbox[move.dst()]];
			piece_hash ^= zobrist_square[move.src()][mailbox[move.dst()]];
		}
		if ((prev.prev_piece() & 7) == PAWN) {
			pawn_hash ^= zobrist_square[move.dst()][prev.prev_piece()];
		} else piece_hash ^= zobrist_square[move.dst()][prev.prev_piece()];
		mailbox[move.src()] = mailbox[move.dst()];
		mailbox[move.dst()] = prev.prev_piece();
		// Update piece and occupancy bitboard
		piece_boards[piece] ^= square_bits(move.src()) | square_bits(move.dst());
	...
	if (zobrist != old_hash || pawn_hash != old_pawn_hash || piece_hash != old_piece_hash) {
		...
		std::cerr << "Or, piece hash mismatch after unmake: expected " << piece_hash << " got " << old_piece_hash << '\n';
