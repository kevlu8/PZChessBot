...
void gen_rook_moves(int sq, Bitboard piece) {
	...
	do {
		...
		Bitboard moves = 0;
		if (west & board)
			moves |= west & ~((1ULL << (63 - __builtin_clzll(west & board))) - 1);
		else
			moves |= west;
		if (south & board)
			moves |= south & ~((1ULL << (63 - __builtin_clzll(south & board))) - 1);
		else
			moves |= south;
		moves |= east & blsmsk(east & board);
		moves |= north & blsmsk(north & board);
...
void gen_bishop_moves(int sq, Bitboard piece) {
	...
	do {
		...
		Bitboard moves = 0;
		if (sw & board)
			moves |= sw & ~((1ULL << (63 - __builtin_clzll(sw & board))) - 1);
		else
			moves |= sw;
		if (se & board)
			moves |= se & ~((1ULL << (63 - __builtin_clzll(se & board))) - 1);
		else
			moves |= se;
		moves |= ne & blsmsk(ne & board);
		moves |= nw & blsmsk(nw & board);
	...
	for (int dst = sq; dst < 64; dst++, board <<= 1) {
		if (board & ne) {
			bishop_blockers[sq][dst] = ne & (blsmsk(board) >> 1);
			...
			bishop_blockers[sq][dst] = nw & (blsmsk(board) >> 1);
			bishop_blockers[dst][sq] = bishop_blockers[sq][dst];
			bishop_blockers_pure[dst][sq] = bishop_blockers_pure[sq][dst] = bishop_blockers[sq][dst];
		}
	}
...
void bishop_moves(const Position &pos, pzstd::vector<Move> &moves) {
	...
	while (pieces) {
		int sq = __builtin_ctzll(pieces);
		uint32_t idx = bishop_magics[sq].offset + pext(pos.piece_boards[OCC(WHITE)] | pos.piece_boards[OCC(BLACK)], bishop_magics[sq].mask);
		Bitboard dsts = bishop_movetable[idx] & ~pos.piece_boards[OCC(pos.side)];
		while (dsts) {
			int dst = __builtin_ctzll(dsts);
			moves.push_back(Move(sq, dst));
			dsts = blsr(dsts);
...
void rook_moves(const Position &pos, pzstd::vector<Move> &moves) {
	...
	while (pieces) {
		int sq = __builtin_ctzll(pieces);
		uint32_t idx = rook_magics[sq].offset + pext(pos.piece_boards[OCC(WHITE)] | pos.piece_boards[OCC(BLACK)], rook_magics[sq].mask);
		Bitboard dsts = rook_movetable[idx] & ~pos.piece_boards[OCC(pos.side)];
		while (dsts) {
			int dst = __builtin_ctzll(dsts);
			moves.push_back(Move(sq, dst));
			dsts = blsr(dsts);
...
void bishop_captures(const Position &pos, pzstd::vector<Move> &moves) {
	...
	while (pieces) {
		int sq = __builtin_ctzll(pieces);
		uint32_t idx = bishop_magics[sq].offset + pext(pos.piece_boards[OCC(WHITE)] | pos.piece_boards[OCC(BLACK)], bishop_magics[sq].mask);
		Bitboard dsts = bishop_movetable[idx] & pos.piece_boards[OPPOCC(pos.side)];
		while (dsts) {
			int dst = __builtin_ctzll(dsts);
			moves.push_back(Move(sq, dst));
			dsts = blsr(dsts);
...
void rook_captures(const Position &pos, pzstd::vector<Move> &moves) {
	...
	while (pieces) {
		int sq = __builtin_ctzll(pieces);
		uint32_t idx = rook_magics[sq].offset + pext(pos.piece_boards[OCC(WHITE)] | pos.piece_boards[OCC(BLACK)], rook_magics[sq].mask);
		Bitboard dsts = rook_movetable[idx] & pos.piece_boards[OPPOCC(pos.side)];
		while (dsts) {
			int dst = __builtin_ctzll(dsts);
			moves.push_back(Move(sq, dst));
			dsts = blsr(dsts);
...
Bitboard rook_attacks(Square sq, Bitboard occ) {
	uint32_t idx = rook_magics[sq].offset + pext(occ, rook_magics[sq].mask);
	return rook_movetable[idx];
}

Bitboard bishop_attacks(Square sq, Bitboard occ) {
	uint32_t idx = bishop_magics[sq].offset + pext(occ, bishop_magics[sq].mask);
...
Bitboard queen_attacks(Square sq, Bitboard occ) {
	...
	idx = bishop_magics[sq].offset + pext(occ, bishop_magics[sq].mask);
...
void Position::update_control() {
	...
	Square w_king_sq = (Square)__builtin_ctzll(w_king);
	Square b_king_sq = (Square)__builtin_ctzll(b_king);
	...
	while (white) {
		Square sq = (Square)__builtin_ctzll(white);
		Bitboard control = rook_attacks(sq, occ ^ b_king);
		side_control[WHITE] |= control;
		white = blsr(white);

		if (__builtin_popcountll(rook_blockers_pure[sq][b_king_sq] & occ) == 1) {
	...
	while (black) {
		Square sq = (Square)__builtin_ctzll(black);
		Bitboard control = rook_attacks(sq, occ ^ w_king);
		side_control[BLACK] |= control;
		black = blsr(black);

		if (__builtin_popcountll(rook_blockers_pure[sq][w_king_sq] & occ) == 1) {
	...
	while (white) {
		...
		if (__builtin_popcountll(bishop_blockers_pure[sq][b_king_sq] & occ) == 1) {
	...
	while (black) {
		...
		if (__builtin_popcountll(bishop_blockers_pure[sq][w_king_sq] & occ) == 1) {
...
bool Position::is_legal(Move move) const {
	Square king_sq = (Square)__builtin_ctzll(piece_boards[KING] & piece_boards[OCC(side)]);
	...
	if (__builtin_popcountll(checkers[side]) > 1)
		return false;

	if (checkers[side]) {
		// Block
		Square checker_sq = (Square)__builtin_ctzll(checkers[side]);
...
next:
	...
	if (pinned[side] & square_bits(move.src())) {
		...
		while (p) {
			...
			Square pinner_sq = (Square)__builtin_ctzll(p);
			Bitboard ray = ((p) & -(p)) | rook_blockers_pure[pinner_sq][king_sq] | bishop_blockers_pure[pinner_sq][king_sq];
